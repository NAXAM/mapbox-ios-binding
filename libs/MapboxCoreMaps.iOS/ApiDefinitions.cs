...
namespace MapboxCoreMaps
{
	...
	interface MBMObservable
	{
		...
		void SubscribeForObserver (MBMObserver observer, string[] events);
		...
		void UnsubscribeForObserver (MBMObserver observer, string[] events);

		// -(void)unsubscribeForObserver:(id<MBMObserver> _Nonnull)observer;
		[Export ("unsubscribeForObserver:")]
		void UnsubscribeForObserver (MBMObserver observer);
	}
	...
	interface MBMStyleManager
	{
		...
		[Verify (MethodToProperty)]
		MBMStyleObjectInfo[] StyleSources { get; }
	...
	interface MBMCameraManager
	{
		...
		[Verify (MethodToProperty)]
		MBMFreeCameraOptions FreeCameraOptions { get; }

		// -(MBMCameraBounds * _Nonnull)getBounds __attribute__((ns_returns_retained));
		[Export ("getBounds")]
		[Verify (MethodToProperty)]
		MBMCameraBounds Bounds { get; }
		...
		// -(id _Nonnull)getMapProjection __attribute__((ns_returns_retained));
		[Export ("getMapProjection")]
		[Verify (MethodToProperty)]
		NSObject MapProjection { get; }
	}
	...
	{
		// -(instancetype _Nullable)initWithUIImage:(UIImage * _Nonnull)uiImage;
	...
	interface MBMMap
	{
		...
		[Verify (MethodToProperty)]
		MBMRenderCacheOptions RenderCacheOptions { get; }
	}
	...
	interface MBMMapMemoryBudget
	{
		...
		// -(MBMMapMemoryBudgetInMegabytes * _Nonnull)getMapMemoryBudgetInMegabytes;
		[Export ("getMapMemoryBudgetInMegabytes")]
		[Verify (MethodToProperty)]
		MBMMapMemoryBudgetInMegabytes MapMemoryBudgetInMegabytes { get; }

		// -(MBMMapMemoryBudgetInTiles * _Nonnull)getMapMemoryBudgetInTiles;
		[Export ("getMapMemoryBudgetInTiles")]
		[Verify (MethodToProperty)]
		MBMMapMemoryBudgetInTiles MapMemoryBudgetInTiles { get; }
	...
	interface MBMRenderedQueryGeometry
	{
		...
		[Static]
		[Export ("fromNSArray:")]
		MBMRenderedQueryGeometry FromNSArray (MBMScreenCoordinate[] value);
		...
		// -(MBMScreenBox * _Nonnull)getScreenBox;
		[Export ("getScreenBox")]
		[Verify (MethodToProperty)]
		MBMScreenBox ScreenBox { get; }

		// -(MBMScreenCoordinate * _Nonnull)getScreenCoordinate;
		[Export ("getScreenCoordinate")]
		[Verify (MethodToProperty)]
		MBMScreenCoordinate ScreenCoordinate { get; }

		// -(NSArray<MBMScreenCoordinate *> * _Nonnull)getNSArray;
		[Export ("getNSArray")]
		[Verify (MethodToProperty)]
...
*/[Protocol]
	...
	interface MBMOfflineRegion
	{
		...
		void SetOfflineRegionObserverForObserver (MBMOfflineRegionObserver observer);
	}
...
*/[Protocol]
	...
	interface MBMCustomLayerHost
	{
		...
		void RenderingWillStart (MTLDevice metalDevice, nuint colorPixelFormat, nuint depthStencilPixelFormat);
		...
		void Render (MBMCustomLayerRenderParameters parameters, MTLCommandBuffer mtlCommandBuffer, MTLRenderPassDescriptor mtlRenderPassDescriptor);
